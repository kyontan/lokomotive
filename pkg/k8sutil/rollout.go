...
func RolloutDaemonSet(dsi clientappsv1.DaemonSetInterface, name string) error {
	generation, err := RolloutRestartDaemonSet(dsi, name)
	if err != nil {
	...
	if err := WaitForDaemonSet(dsi, name, options); err != nil {
...
func RolloutDeployment(deployClient clientappsv1.DeploymentInterface, name string) error {
	generation, err := RolloutRestartDeployment(deployClient, name)
	if err != nil {
		return fmt.Errorf("restarting: %w", err)
	}
	...
	if err := WaitForDeployment(deployClient, name, options); err != nil {
...
func WaitForDaemonSet(dsi clientappsv1.DaemonSetInterface, name string, options WaitOptions) error {
	...
	if _, err := watchtools.UntilWithSync(context.TODO(), lw, &appsv1.DaemonSet{}, nil, watchF); err != nil {
...
func WaitForDeployment(deployClient clientappsv1.DeploymentInterface, name string, options WaitOptions) error {
	...
	if _, err := watchtools.UntilWithSync(context.TODO(), lw, &appsv1.Deployment{}, nil, watchF); err != nil {
